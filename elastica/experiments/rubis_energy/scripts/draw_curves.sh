...
export SCRIPTS_PATH=$PROJECT_PATH/experiments/rubis_energy/scripts
...
ymax=250
xmax=`expr $xmax / 1000`
for l in $(ls $EXP_PATH/*.watt.log)
do
	server_name=$(basename $l | cut -d. -f1)
	metric="energy-$server_name"


	$SCRIPTS_PATH/gen_gnuplot_energy.sh "$l" "$metric" "$CURVE_PATH" " " "Power Consumption (W)" "3" "" "$ymax" "$xmax"
	gnuplot $CURVE_PATH/$metric.gnuplot
	ps2pdf $CURVE_PATH/$metric.ps $CURVE_PATH/$metric.pdf
done
